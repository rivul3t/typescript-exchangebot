...
export class StartCommand implements BotCommand {
  ...
  async execute(): Promise<TelegramResponse> {
    const response = await this.bot.view.sendMessage(
      "Привет, я миниботик. Отправь  /currency и я дам тебе курс валюты, который ты захочешь",
      this.chatId,
    );
    return response;
  }
...
export class CurrencyCommand implements BotCommand {
  ...
  async execute(): Promise<TelegramResponse> {
    const response = await this.bot.view.sendMessage(
      "Укажи валюту в формате ВАЛЮТА-ВАЛЮТА",
      this.chatId,
    );
    this.bot.userStates[this.chatId] = new SendingCurrencyUserState(
      this.bot,
      this.chatId,
    );
    return response;
